import {
    ...
    DRAWING_TAG_ID_OPTIONS,
    TAG_ID_OPTIONS,
    TAG_SETTINGS_ID_OPTIONS,
    defaultState
...
import { createApi } from './setup-api.js';
import { createResponsesManager } from './setup-responses.js';
import { createEventHandlers } from './setup-events.js';
...
const store = {
    ...
    get selectedResponseFilenames() {
        return selectedResponseFilenames;
    },
    set selectedResponseFilenames(value) {
        selectedResponseFilenames = value;
    },
    ...
    set sortableInstance(value) {
        sortableInstance = value;
    },
    get responsesRefreshTimer() {
        return responsesRefreshTimer;
    },
    set responsesRefreshTimer(value) {
        responsesRefreshTimer = value;
    }
};
...
const responsesManager = createResponsesManager({ store, els });
const {
    renderResponses,
    loadResponses,
...
const { initEvents } = eventHandlers;
...
function updateStickerCount(nextCount) {
    ...
    while (colors.length < count) {
        const fallback = DEFAULT_STICKER_CONFIG.colors[colors.length] || '#cccccc';
